...
layout(push_constant) uniform PC {
    ...
    // p[4]    = quality  (0..1, scales the 20 volumetric steps)
...
void main() {
    ...
    int   volsteps = int(20.0 * clamp(pc.p[4], 0.05, 1.0));
    ...
    vec2 shifted = (fragUV - origin) / (2.0 * unit * scale) + 0.5;
    vec2 uv = vec2(shifted.x, 1.0 - shifted.y) - 0.5;
    uv.y *= R.y / R.x;
