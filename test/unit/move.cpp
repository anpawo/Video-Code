...
TEST(MoveTransformationTest, MoveToNewPosition) {
    ...
    transformation::move(input, *(new Register()), args);
...
TEST(MoveTransformationTest, MoveWithMultipleFrames) {
    std::vector<Frame> frames;
    frames.emplace_back(cv::Mat::zeros(10, 10, CV_8UC4), Metadata{0, 0});
    frames.emplace_back(cv::Mat::zeros(10, 10, CV_8UC4), Metadata{0, 0});
    auto input = std::make_shared<MockInput>(std::move(frames));
    ...
    EXPECT_EQ(meta1.x, 50);
    EXPECT_EQ(meta1.y, 50);
