...
{
public:
    explicit WebImage(nlohmann::json::object_t&& args)
    ...
    cv::Mat getBaseMatrix(const nlohmann::json::object_t&) override
    {
        return _base;
    }
...
static std::unique_ptr<IInput> createWebImage(nlohmann::json::object_t&& args)
{
    return std::make_unique<WebImage>(std::move(args));
}

extern "C" void vc_register_plugin(VC::PluginRegistrar* registrar)
{
    registrar->registerInput(registrar->context, "WebImage", createWebImage);
}
