...
para.waitFor(g)
para.curY = 0
para.newline().newline().wait(PAUSE_DELAY).add(Code("s = Square()"))
...
para.waitFor(s).wait(PAUSE_DELAY).add(Code("s.moveBy(x=-1)"))
timestamp("show: move square")
s.waitFor(para).moveBy(x=-1.25)

timestamp("text: circle")
para.waitFor(s).wait(PAUSE_DELAY).add(Code("c = Circle()"))
...
para.waitFor(c).wait(PAUSE_DELAY).add(Code("c.moveBy(x=1)"))
timestamp("show: move circle")
c.waitFor(para).moveBy(x=1.25)

timestamp("text: group")
para.newline().waitFor(c).wait(PAUSE_DELAY).add(Code("g = Group(s, c)"))
timestamp("text: group rotation")
para.add(Code("g.rotateBy(180)"))
...
para.waitFor(g).wait(PAUSE_DELAY).add(Code("g.scaleTo(0.5).rotateBy(180)"))
