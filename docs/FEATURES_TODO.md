...
| **X11** | **Fait — `69c3258`, cinq goldens à ton œil.** Sous `Context.noRegister()`, `.position()` n'écrivait plus `meta.position` : depuis `428c0f6` (24 août) `apply()` sortait de la boucle avant `modify()` pour un élément sans slot. `CompoundPolygon` lit cette position pour poser ses contours, donc les 103 lignes d'un `Plane` s'empilaient au centre — une croix brillante au lieu d'une grille pâle, visible dans le gif du README — et les lettres de la scène `matte` perdaient leur place (mot large de 1339 px au lieu de 1285, la largeur d'un `Text` ordinaire). Deux lignes dans `input.py`, `test/no_register_position_test.py` échoue sans elles. Empreinte inchangée. Les goldens `gradient-holes`, `matte`, `resize` (0 et 15) et `video` enregistraient le bug : à réécrire une fois vus. | Le gif du README | 30 min | — |
