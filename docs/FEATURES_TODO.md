...
| **X8** | **`Text("videocode")` seul dessine une bouillie.** Les lettres n'obtiennent leur place que si le texte est DÉPLACÉ ailleurs qu'à l'origine : la mise en page existe côté Python, mais aucune revendication de position n'est écrite tant que la valeur ne change pas, et le C++ met alors chaque lettre à (0, 0). Mesuré : `Text(text="Kate").position(0.001, 0)` donne quatre x distincts (-0,366 / -0,125 / 0,078 / 0,216) ; `.position(0, 0)`, `.fadeIn()` et le texte laissé tel quel n'en donnent aucun, et les quatre lettres se superposent sur 25 px au lieu de s'étaler. Le corpus ne l'attrape pas parce que toutes ses scènes placent leur texte ailleurs qu'au centre exact. **Le correctif écrit la position des lettres à la création — donc il déplace l'empreinte des 50 scènes, et ça se demande avant de le faire.** | La première ligne qu'on écrit | 2 h | ton feu vert |
