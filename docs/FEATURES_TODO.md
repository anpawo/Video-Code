...
| **X6** | **Pas commité — le correctif honnête bouge une scène.** `Polygon.width` et `Group.width` ignorent `meta.scale` : un `Square(2).scale(2)` est mesuré 2. Correctif écrit (Polygon/Rectangle/RightTriangle/CompoundPolygon lisent et écrivent la taille DESSINÉE, Text garde la géométrie pour sa mise en page, RectangleDimensions perd sa double compensation) : `Square(2).scale(2).width` → 4, `Circle(1).scale(3)` → 6, un Group avec membre ×2 → 9. Mais `_MemberBase.width` suit, donc le pivot de `stateful_group_scale.py` passe de x=0 (centre des boîtes NON mises à l'échelle) à x=-0.25 (centre des boîtes dessinées) : rect -1.5→-1.375, cercle 1.5→1.625, digest `077096be`→`f97c9366`. Laissé non commité dans l'arbre (8 fichiers, tests X6 dans `test/layout_test.py`) ; le golden est à regarder avant d'accepter | Premier `nextTo` | ½ j | un golden à regarder |
...
| # | Ce que c'est | Taille | Dépend de |
|---|---|---|---|
| **B1** | **Le curseur de texte est une tête de lecture** — la ligne où tu es allume sa barre ; une touche joue depuis là. Aucune écriture, que de la lecture | S | — |
...
| **B5** | **Éditeur de courbes** — une courbe tirable sur chaque effet, qui écrit `easing=CubicBezier(...)`. Les presets et les courbes à main sont déjà le même type | M | — |
| **B6** | **Les repères** — `timestamp()` est écrit 30 fois dans le tutoriel et n'est **jamais dessiné**. Le C++ sait déjà sauter de l'un à l'autre. Il manque trois lignes dans `sceneModel` | S | — |
| **B7** | **Ce qu'un geste doit refuser** — badge « ×3 » sur un clip fait dans une boucle, refus sur un autre fichier (X5), et l'inverse constructif : si l'argument est une constante nommée, proposer de changer **la constante** | S puis M | X4, X5 |

### C — Sortie, agent, réutilisation

| # | Ce que c'est | Taille | Dépend de |
|---|---|---|---|
| **C1** | **L'agent regarde l'image qu'il vient de faire** — il modifie, rend la frame (9 ms, sans fenêtre), l'ouvre, et dit que le titre déborde. Demande `--frame N` / `--at 1.5`, et une planche-contact `--sheet` | S | A7 |
| **C2** | **L'agent sait où tu es** — aujourd'hui il ne reçoit **que ton texte** : ni le fichier, ni ta ligne, ni la sélection, ni la tête de lecture, ni les avertissements du dernier run. Plus un `--inspect` qui imprime le modèle de scène en JSON | S | — |
| **C3** | **Une scène, tous les formats** — `--for youtube,tiktok,square`. La scène **se remet en page** (`setScreen`, `Split.AUTO`) au lieu d'être recadrée. Plus la normalisation de loudness | M | — |
| **C4** | **Exporter depuis l'éditeur** — un bouton, une barre de progression, la plage entre les repères. L'éditeur ne sait aujourd'hui **pas exporter du tout**. La décision est déjà écrite dans `BACKLOG.md` : même binaire, progression lue sur la sortie | M | A7 |
| **C5** | **Tes propres modèles** — un dossier `templates/` dans ton projet, découvert dans le panneau avec ses champs. La machinerie existe, elle ne regarde que le paquet de la librairie | S | — |
| **C6** | **Chapitres et miniature** — chaque `timestamp()` devient un chapitre dans le fichier et une ligne à coller sur YouTube | S | B6, C1 |
